...
/// Pseudo-random 8bit number generator
...
int program_start() {
    ...
    while(1) {
        uint16_t start_ptr = rand8();
        int status1 = enc28j60_writeframe(&eth0, 
        ...
        int status2 = enc28j60_init_txbuffer(&eth0, start_ptr, sizeof(tx_frame));

        printf("Transmitting...\t");
        int status3 = enc28j60_transmitframeblocking(&eth0, start_ptr, sizeof(tx_frame));
        printf("done\t Status %u %u %u\n", status1, status2, status3);
        fflush(stdout);
