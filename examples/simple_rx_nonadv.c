...
int program_start() {
    ...
    while(1) {
        int status = enc28j60_receiveframeblocking(&eth0, (enc28j60_header*) frame, sizeof(frame), 0);
        printf("Receive status %u\n", status);
        if(status == 0) {
            char *frame_start = &frame[sizeof(enc28j60_header)];
            unsigned length = eth0.length - sizeof(enc28j60_header);

            printf("\tFrame length %u\n", length);
            printf("\tFirst byte is %02X, the last is %02X\n", frame_start[0], frame_start[length-1]);

            enc28j60_ethernet_header *header = (enc28j60_ethernet_header *) frame_start;
